...
def solution(record):
    ...
    for i in record:
        data = i.split(' ')
        if data[0] == 'Change' or data[0] =='Enter':
            nick_dic[data[1]] = data[2]

        if data[1] not in nick_dic:
            nick_dic[data[1]] = data[2]

    # print(nick_dic)

    for make_all in record:
        data_second = make_all.split(' ')
        if data_second[0] == 'Enter':
            answer.append(nick_dic[data_second[1]]+'님이 들어왔습니다.')
        elif data_second[0] == 'Leave':
            answer.append(nick_dic[data_second[1]]+'님이 나갔습니다.')
