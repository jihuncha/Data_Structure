...
def solution(orders, course):
    ...
    for count in course:
        temp_dic = {}
        for i in orders:
            # combination을 check 에 담는다
            check = list(itertools.combinations(i, count))
            print(check)
            for j in check:
                # list형태로 변경
                temp_list = list(set(''.join(j)))
                # temp_list 정렬
                temp_list.sort()
                # join사용해서 합쳐 준다.
                temp_str = ''.join(temp_list)
                # dic 생성
                if temp_str not in temp_dic:
                    temp_dic[temp_str] = 1
                else:
                    temp_dic[temp_str] += 1
        # dic이 존재하면서 maxvalue가 2이상인 경우네는
        if temp_dic and max(temp_dic.values()) >= 2:
            temp_max = max(temp_dic.values())
            # 해당 아이템 대입
            for j in temp_dic.items():
                if j[1] == temp_max:
                    answer.append(j[0])
        # 최종 결과도 정렬해준다.
        answer.sort()
