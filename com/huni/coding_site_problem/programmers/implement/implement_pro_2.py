...
####################################################
# 1. rotate 어떻게 해야할지 30~40분 넘게 걸림
# 2. rotate 방법은 찾았으나......
# 3. 일단 생각은 lock에서 0 (돌기 부분) 의 index를 도출해낸다
# 4. 그 다음에 key를 상하좌우로 이동하면서 key의 1 (홈) 부분과 lock의 돌기부분 인덱스가 전부 동일한 것이 있는지 체크해본다
# 5. 없는 경우는 rotate하여 4부분을 반복한다....
# 6. 근데 왜 fail뜨는지 모르겠다 ㅠ
####################################################
...
def solution(key, lock):
    ...
    def check_and_move(search_key):
        ...
        for i in range(4):
            ...
            # 결과를 발견한 경우
            if len(temp_key_list) == len(suc_list) and sorted(temp_key_list)[:] == sorted(suc_list)[:]:
                answer = True
                return
...
# 테스트 2 〉	실패 (0.16ms, 10.3MB)
# 테스트 3 〉	실패 (0.87ms, 10.3MB)
# 테스트 4 〉	실패 (0.11ms, 10.4MB)
# 테스트 5 〉	실패 (2.43ms, 10.4MB)
# 테스트 6 〉	실패 (0.37ms, 10.4MB)
# 테스트 7 〉	실패 (0.71ms, 10.2MB)
# 테스트 8 〉	실패 (1.13ms, 10.3MB)
# 테스트 9 〉	실패 (1.50ms, 10.3MB)
# 테스트 10 〉	실패 (2.08ms, 10.3MB)
# 테스트 11 〉	실패 (1.77ms, 10.3MB)
# 테스트 12 〉	실패 (0.51ms, 10.3MB)
# 테스트 13 〉	실패 (0.63ms, 10.3MB)
# 테스트 14 〉	실패 (0.20ms, 10.3MB)
# 테스트 15 〉	실패 (0.37ms, 10.3MB)
# 테스트 16 〉	실패 (0.63ms, 10.4MB)
# 테스트 17 〉	실패 (1.08ms, 10.3MB)
# 테스트 18 〉	실패 (0.42ms, 10.2MB)
# 테스트 19 〉	실패 (0.37ms, 10.3MB)
# 테스트 20 〉	실패 (0.92ms, 10.3MB)
# 테스트 21 〉	실패 (1.07ms, 10.3MB)
# 테스트 22 〉	실패 (0.84ms, 10.4MB)
# 테스트 23 〉	통과 (0.20ms, 10.3MB)
# 테스트 24 〉	통과 (0.15ms, 10.3MB)
# 테스트 25 〉	통과 (0.81ms, 10.3MB)
# 테스트 26 〉	통과 (1.47ms, 10.3MB)
# 테스트 27 〉	통과 (1.58ms, 10.3MB)
# 테스트 28 〉	통과 (0.57ms, 10.4MB)
# 테스트 29 〉	통과 (0.22ms, 10.3MB)
# 테스트 30 〉	통과 (0.64ms, 10.3MB)
# 테스트 31 〉	통과 (0.47ms, 10.3MB)
# 테스트 32 〉	통과 (1.95ms, 10.3MB)
# 테스트 33 〉	통과 (0.70ms, 10.3MB)
# 테스트 34 〉	실패 (0.20ms, 10.4MB)
# 테스트 35 〉	실패 (0.21ms, 10.3MB)
# 테스트 36 〉	통과 (0.78ms, 10.4MB)
# 테스트 37 〉	통과 (0.49ms, 10.2MB)
# 테스트 38 〉	실패 (0.47ms, 10.2MB)