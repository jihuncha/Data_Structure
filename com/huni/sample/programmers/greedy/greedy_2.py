...
####################################################################################################
# 미해결 but 생각 공유
# 1. 상하이동 -> ord() 사용하여 아스키코드 변환하여 A와의 차이 만큼 이동
# 2. 좌우이동
#   2-1. 처음에는 되돌아가는 (즉 우로 갔다가 좌로 가는 경우) 경우의 수가 절대 존재하지 않을거라고 생각하고 문제 해결
#   2-2. set을 활용하여 나머지가 전부 a인경우는 나머지칸 만큼 따로 길이를 더해주는 형태로 하려 했으나 ZZAAAZZ 를 해결 못해서 포기
####################################################################################################
